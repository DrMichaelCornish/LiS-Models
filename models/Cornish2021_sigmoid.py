...
class Cornish2021_sigmoid(BaseModel):
    """
    ...
    def __init__(self, options=None, name="Cornish & Marinescu (2021) Zero Dimensional Model"):
        ...
        # Model parameters
        #######################################
        from .parameters.cornish2021_sigmoid_parameters import Cornish2021_sigmoidParameters
        self.param = Cornish2021_sigmoidParameters()
        param = self.param
        ...
        sigmoid_H = ( (I>=0)*1/ (1+ gamma*pybamm.exp(-alpha*S8) ) ) + ((I<0)*1/(1+ gamma*pybamm.exp(-alpha*S4)))
        #( (1- gamma*pybamm.exp(-alpha*S8))*(I>=0) ) + ((1- gamma*pybamm.exp(-alpha*S4))*(I<0))
        
        #1/(1 + gamma*pybamm.exp(-alpha*S8))
        i_H = -2 * ih0 * ar * sigmoid_H * pybamm.sinh(iH_coef * eta_H)
        
        #sigmoid_M = ( (1- gamma*pybamm.exp(-alpha*S4))*(I>=0) ) + ((1- gamma*pybamm.exp(-alpha*S2))*(I<0))
        sigmoid_M = ((I>=0)*1/(1+ gamma*pybamm.exp(-alpha*S4))) + ((I<0)*1/(1+ gamma*pybamm.exp(-alpha*S2)))
        i_M = -2 * im0 * ar * sigmoid_M * pybamm.sinh(iM_coef * eta_M)

        # Low plateau current [A] as defined by equation (5b) in [1]
        #sigmoid_L =( (1- gamma*pybamm.exp(-alpha*S2))*(I>=0) ) + ((1- gamma*pybamm.exp(-alpha*S))*(I<0))
        sigmoid_L = ((I>=0)*1/(1+ gamma*pybamm.exp(-alpha*S2))) + ((I<0)*1/(1+ gamma*pybamm.exp(-alpha*S)))
        i_L = -2 * il0 * ar * sigmoid_L * pybamm.sinh(iL_coef * eta_L)
    ...
    def default_parameter_values(self):
        ...
        file = "models/inputs/parameters/lithium-sulfur/cornish2021_sigmoid_parameters.csv"
